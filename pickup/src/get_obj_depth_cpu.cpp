...
private:
    ...
    int inpWidth = 320;  // Width of network's input image 416
    int inpHeight = 320; // Height of network's input image 416
...
//            qPose.setRPY(0, 0, 0);
    ...
    cv::Scalar getObjPose(int bottleNum, int x, int y, int width, int height, Mat& frame, Mat& depth){
        ...
        printf("bottle%dAvgPose:(%f %f %f)\n\n", bottleNum, avgX, avgY, avgDepth);
        // 图像中显示各瓶子空间位置
        oss << bottleName << "Pose: (" << avgX << " " << avgY << " "  << avgDepth << ")";
        cv::putText(frame, oss.str(), cv::Point(250, 15*bottleNum), cv::FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255, 0, 0), 1, CV_AA);
...
//        }
    ...
    void postprocess(Mat& frame, const vector<Mat>& outs, Mat& depth)
    {
        vector<int> classIds;
        ...
        for (size_t i = 0; i < indices.size(); ++i)
        {
            ...
            if (classes[classIds[idx]] == "bottle" && confidences[idx] > 0.4) {
                bottleNum ++;
                getObjPose(bottleNum, box.x, box.y, box.width, box.height, frame, depth);
            }
        }
    }
    ...
    void loadNetwork(void){
        ...
        string classesFile = "/home/sdhm/catkin_ws/src/pickup/data/coco.names";
        ifstream ifs(classesFile.c_str());
        string line;
        while (getline(ifs, line)) classes.push_back(line);

        // Give the configuration and weight files for the model
        string modelConfiguration = "/home/sdhm/catkin_ws/src/pickup/data/yolov3.cfg";
        string modelWeights = "/home/sdhm/catkin_ws/src/pickup/data/yolov3.weights";
    ...
    void imageDetector()
    {
        ...
        for(; running && ros::ok();){
            if(updateImage){
                ...
                /************************************ 图像处理 ************************************/
//                resize(color, color, Size(), 0.5, 0.5); // resize image
                blobFromImage(color, blob, 1/255.0, cvSize(inpWidth, inpHeight), Scalar(0,0,0), true, false); // Create a 4D blob from a frame.
                net.setInput(blob); //Sets the input to the network
                vector<Mat> outs;
                net.forward(outs, getOutputsNames(net)); // Runs the forward pass to get output of the output layers
                postprocess(color, outs, depth); // Remove the bounding boxes with low confidence 并获取目标中心点深度信息
