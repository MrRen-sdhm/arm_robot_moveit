...
def draw_boxes_cv(image, boxes, scores, labels, classes, detection_size,
               font='./data/font/FiraMono-Medium.otf', show=True):
    """
    :param boxes, shape of  [num, 4]
    :param scores, shape of [num, ]
    ...
    detection_size, original_size = np.array(detection_size), np.array(image.shape[0:2])[::-1]
    ratio = original_size / detection_size
    # print "\nbefore", boxes
    for i in range(len(labels)): # for each bounding box, do:
        bbox, score, label = boxes[i], scores[i], classes[labels[i]]
        ...
        cv2.rectangle(image, (left, top), (right, bottom), colors[labels[i]], 2)

        # Display the label at the top of the bounding box
        labelSize, baseLine = cv2.getTextSize(bbox_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        top = max(top, labelSize[1])
        cv2.rectangle(image, (left-1, top - int(1.2*labelSize[1])), (left + int(1.0*labelSize[0]), 
                    top + baseLine), colors[labels[i]], cv2.FILLED)
        cv2.putText(image, bbox_text, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1, cv2.LINE_AA)
